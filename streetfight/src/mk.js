window.fgm = ()=> {
  var canvasLightning = function(c, cw, ch){
    ...
    /*=============================================================================*/
      this.lightningTimer = function(){
        ...
        if(this.lightTimeCurrent >= this.lightTimeTotal){
          var newX = this.rand(0, cw - 100);
          var newY = this.rand(0, ch / 2); 
          var createCount = this.rand(1, 2);
          while(createCount--){							
    ...
    /*=============================================================================*/
      this.loop = function(){
            var loopIt = function(){
          _this.requestId = requestAnimationFrame(loopIt, _this.c);
          _this.clearCanvas();
          _this.updateL();
  ...
  mk.controllers.Base = function (options) {
    ...
    var a = options.arena;
    var cl = new canvasLightning(a.canvas, a.width, a.height);				
    setupRAF();
    cl.init();
    this.cl =cl
  ...
  mk.reset = function () {
    ...
    window.cancelAnimationFrame(game.cl.requestId);
    game.cl = null
    game.fighters = null;
  ...
  mk.controllers.Basic.prototype._moveFighter 
  ...
  mk.controllers.Multiplayer.prototype._keydown = function (keycode){
    try{
      var self = this
      let f1 = this.fighters[0];
      let f2 = this.fighters[1];
      mk.pressed[keycode] = true;
      var move = self._getMove(mk.pressed, mk.controllers.keys.p1, 0);
      self._moveFighter(f1, move);
      move = self._getMove(mk.pressed, mk.controllers.keys.p2, 1);
      self._moveFighter(f2, move);
    }catch(e){}
  }

  mk.controllers.Multiplayer.prototype._keyup = function (keycode,figters){
    try{
      var self = this
      let f1 = this.fighters[0];
      let f2 = this.fighters[1];
      delete mk.pressed[keycode];
      var move = self._getMove(mk.pressed, mk.controllers.keys.p1, 0);
      self._moveFighter(f1, move);
      move = self._getMove(mk.pressed, mk.controllers.keys.p2, 1);
      self._moveFighter(f2, move);
  ...
  mk.arenas.Arena = function (options) {
    ...
    this._context = options.canvas.getContext();
...
/* * * * * * * * * * * * * * * * Definition of all movements * * * * * * * * * * * * * * * */
  ...
  mk.moves.ForwardJump.prototype._action = function () {
    if (this._currentStep > (this._totalSteps - 1) / 2) { //Move down
      ...
      this.owner.setX(this.owner.getX() + 24.5);
    } else { //Move up
      this.owner.setY(this.owner.getY() - 26);
      this.owner.setX(this.owner.getX() + 24.5);
    }
...
/* * * * * * * * * * * * * End of the movements definition * * * * * * * * * * * * * * */
  ...
  mk.fighters.Fighter.prototype.init = function (callback) {
    ...
    this.moves[mk.moves.types.KNOCK_DOWN] = new mk.moves.Endure(this);
